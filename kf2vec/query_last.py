...
def query_func_last(features_folder, features_csv_file_list, model_file, classes, seed, output_folder):
    ...
    level = logging.INFO
    format = '%(message)s'
    handlers = [logging.FileHandler(os.path.join(output_folder, 'last_query_run.log'), 'w+'), logging.StreamHandler()]
    ...
    for c in class_count:
        ...
        state = torch.load(os.path.join(model_file, "last_model_subtree_{}.ckpt".format(c)))
        ...
        # Read embeddings
        df_embeddings = pd.read_csv(os.path.join(model_file, 'last_embeddings_subtree_{}.csv'.format(c)), sep="\t",
        ...
        with torch.no_grad():
            ...
            for features_csv_file in current_class_ids[c]:
                ...
                df_outputs.to_csv(os.path.join(output_folder, 'apples_input_di_mtrx_last_query_{}.csv'.format(features_csv_file)), index=False, sep='\t')
